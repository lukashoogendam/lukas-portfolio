...
> De scripts staan in **`scripts/`** en draai je vanuit de **root van de repo** (`lukas-portfolio/`).
...
```bash
node scripts/add-asset.js --project <slug> --file <pad> --title <titel> [--type image|document]
```
...
node scripts/add-asset.js --project firsfs --file ~/Downloads/rapport.pdf --title "Onderzoeksrapport"

# Screenshot toevoegen
node scripts/add-asset.js --project firsfs --file ~/Desktop/scherm.png --title "Hoofdscherm"

# Extensie niet herkend? Geef type mee
node scripts/add-asset.js --project mijn-project --file ~/data.csv --title "Dataset" --type document
```
...
3. Voegt een nieuwe entry toe aan `images` (`{ title, imageUrl }`) of `documents` (`{ title, url }`) in `frontend/public/data/projects/<slug>.json`.

> Sinds de data-model-refactor zijn er geen `id`/`sortOrder`-velden meer; de volgorde
> volgt de array-volgorde. Pad-validatie en het slug-helpertje worden gedeeld via
> `cli-lib.js` (samen met `add-project.js`).
