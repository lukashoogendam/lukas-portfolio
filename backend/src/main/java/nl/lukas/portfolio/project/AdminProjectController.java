...
public class AdminProjectController {
    ...
    @PostMapping("/reorder")
    @Operation(summary = "Reorder projects", description = "Update the sort order of projects")
    public ResponseEntity<ApiResponse<Void>> reorderProjects(@RequestBody List<String> slugs) {
        projectService.reorderProjects(slugs);
        return ResponseEntity.ok(ApiResponse.<Void>builder()
                .status(200)
